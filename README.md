...
The engine reads the element's rotation (via `transform`) when available and falls back to its bounding box and `data-slope` direction to compute collisions and sliding.

Curved slopes can be defined with a custom function. Set `data-slope="function"` and provide a JavaScript expression via `data-slope-fn` that maps a normalized horizontal position `x` (0–1) to a normalized height `y` (0 at the bottom, 1 at the top):

```html
<div class="object solid slope" data-slope="function" data-slope-fn="Math.pow(x,2)"
